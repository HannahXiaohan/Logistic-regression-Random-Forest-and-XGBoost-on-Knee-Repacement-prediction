...
This script uses a participant-level train/validation/test split, so knees from
the same participant are kept in the same split. Random Forests do not produce
odds ratios, so the odds-ratio column is reported as "n/a (RF)".
...
def split_by_participant(data: pd.DataFrame) -> pd.DataFrame:
    """Assign train/validation/test splits by participant ID."""
    participant = (
        data.groupby("ID", as_index=False)["kr_5yr"]
        .max()
        .rename(columns={"kr_5yr": "participant_event"})
    )
    ...
    train_ids, temp_ids = train_test_split(
        ...
        random_state=RANDOM_STATE,
        stratify=participant["participant_event"],
    )
    temp = participant[participant["ID"].isin(temp_ids)].copy()
    val_fraction_of_temp = VALIDATION_SIZE / (VALIDATION_SIZE + TEST_SIZE)
    val_ids, test_ids = train_test_split(
        temp["ID"],
        train_size=val_fraction_of_temp,
        random_state=RANDOM_STATE,
        stratify=temp["participant_event"],
    )

    split = pd.Series("train", index=data.index, dtype="object")
    split.loc[data["ID"].isin(val_ids)] = "validation"
    split.loc[data["ID"].isin(test_ids)] = "test"
...
def fit_and_predict_split(data: pd.DataFrame, feature_cols: list[str]) -> pd.Series:
    """Fit on train split and predict probabilities for all splits."""
    train = data[data["split"].eq("train")]
    X_train = train[feature_cols]
    y_train = train["kr_5yr"].astype(int)
    ...
    for split_name in ["train", "validation", "test"]:
        mask = data["split"].eq(split_name)
        preds.loc[mask] = model.predict_proba(data.loc[mask, feature_cols])[:, 1]
    return preds
...
def base_row(split_name: str, data: pd.DataFrame, p_base: pd.Series) -> dict[str, object]:
    y = data["kr_5yr"]
...
def main() -> None:
    ...
    analytic = split_by_participant(analytic)
    ...
    split_counts = (
        analytic.groupby("split")
        .agg(knees=("kr_5yr", "size"), participants=("ID", "nunique"), kr_5yr=("kr_5yr", "sum"))
        .reindex(["train", "validation", "test"])
    )

    print("Fitting Base Model RF on train split...")
    p_base = fit_and_predict_split(analytic, feature_sets["base"])
    print("Fitting Base + KL (ordinal) RF on train split...")
    p_kl = fit_and_predict_split(analytic, feature_sets["kl"])
    print("Fitting Base + esKOA (original) RF on train split...")
    p_eskoa_orig = fit_and_predict_split(analytic, feature_sets["eskoa_original"])
    print("Fitting Base + esKOA (alternative) RF on train split...")
    p_eskoa_alt = fit_and_predict_split(analytic, feature_sets["eskoa_alternative"])

    summary_rows = []
    for split_name in ["validation", "test"]:
        mask = analytic["split"].eq(split_name)
        split_data = analytic.loc[mask]
        split_p_base = p_base.loc[mask]
        summary_rows.extend(
            [
                base_row(split_name, split_data, split_p_base),
                experiment_row(
                    split_name,
                    "Base + KL (ordinal) RF",
                    split_data,
                ...
                experiment_row(
                    split_name,
                    "Base + esKOA (original) RF",
                    split_data,
                    split_p_base,
                    p_eskoa_orig.loc[mask],
                ),
                experiment_row(
                    split_name,
                    "Base + esKOA (alternative) RF",
                    split_data,
                    split_p_base,
                    p_eskoa_alt.loc[mask],
                ),
            ]
        )
    ...
    print(f"Split proportions: train={TRAIN_SIZE:.0%}, validation={VALIDATION_SIZE:.0%}, test={TEST_SIZE:.0%}")
    print("\nSplit counts")
