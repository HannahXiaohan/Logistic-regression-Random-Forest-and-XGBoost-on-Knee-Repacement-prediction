"""
XGBoost version of the Table 2 baseline experiments:
    Base Model
    Base + KL (ordinal)
    Base + esKOA (original)
...
OUT_SUMMARY = Path("table2_xgboost_summary.csv")
OUT_PREDICTIONS = Path("table2_xgboost_predictions.csv")
...
def make_xgboost_pipeline(feature_cols: list[str], y_train: pd.Series) -> Pipeline:
    ...
    return Pipeline([("preprocess", preprocessor), ("xgb", xgb)])
...
def grouped_cv_predict_proba(data: pd.DataFrame, feature_cols: list[str]) -> pd.Series:
    ...
    for fold, (train_idx, test_idx) in enumerate(splitter.split(X, y, groups), start=1):
        ...
        model = make_xgboost_pipeline(feature_cols, y.iloc[train_idx])
...
def base_row(data: pd.DataFrame, p_base: pd.Series) -> dict[str, object]:
    ...
    no_kr_mask = y.eq(0)
    return {
        "Model": "Base Model XGBoost",
        "AUC": auc,
        "Hosmer-Lemeshow p-value": hl_p,
        "Odds Ratio for New Variable (95% CI)": "n/a (XGBoost)",
...
def main() -> None:
    ...
    print("Fitting Base Model XGBoost...")
    p_base = grouped_cv_predict_proba(analytic, feature_sets["base"])
    print("Fitting Base + KL (ordinal) XGBoost...")
    p_kl = grouped_cv_predict_proba(analytic, feature_sets["kl"])
    print("Fitting Base + esKOA (original) XGBoost...")
    p_eskoa_orig = grouped_cv_predict_proba(analytic, feature_sets["eskoa_original"])
    print("Fitting Base + esKOA (alternative) XGBoost...")
    p_eskoa_alt = grouped_cv_predict_proba(analytic, feature_sets["eskoa_alternative"])

    summary = pd.DataFrame(
        [
            base_row(analytic, p_base),
            experiment_row("Base + KL (ordinal) XGBoost", analytic, p_base, p_kl),
            experiment_row("Base + esKOA (original) XGBoost", analytic, p_base, p_eskoa_orig),
            experiment_row("Base + esKOA (alternative) XGBoost", analytic, p_base, p_eskoa_alt),
        ]
    )

    predictions = analytic.assign(
        predicted_probability_base_xgboost=p_base,
        predicted_probability_base_plus_kl_xgboost=p_kl,
        predicted_probability_base_plus_eskoa_original_xgboost=p_eskoa_orig,
        predicted_probability_base_plus_eskoa_alternative_xgboost=p_eskoa_alt,
    ...
    print("\nXGBoost Table 2 baseline experiments")
    print("=" * 42)
    ...
    print(f"XGBoost trees: {N_ESTIMATORS}")
